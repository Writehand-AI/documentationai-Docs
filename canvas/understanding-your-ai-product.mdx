...
description: >-
  The AI Product node is the heart of every product on Productised — here's what
...
The **Language** setting sits just above the system prompt, under **Collection Instructions**. It sets the one language your product speaks, all the way through: the greeting, every question and answer option, the analysis your AI Agents write, and the result page at the end.

- **Auto** (the default) uses the language your instructions are written in. The option shows what it detected, for example *Auto (German, from your instructions)*. Most products never need to change it.
- **Choose a language** (English, German, French, Spanish, Italian, Dutch, or Portuguese) to fix it, whatever language your instructions are written in. This is handy when you'd rather write the instructions in English but run the product in German.

<Callout kind="tip">
If a product ever opens in the wrong language, set Language explicitly instead of adding "reply in German" to your prompt. The setting is applied last, after all of Productised's built-in instructions, so it always wins.
</Callout>
...
<Columns cols="3">
  <Card title="AI Judgement" icon="cpu" horizontal="false">
  ...
  <Card title="AI Scorecard" icon="bar-chart-2" horizontal="false">
  ...
  <Card title="Advanced Manual" icon="sliders" horizontal="false">
