...
<Columns cols="2">
  ...
  <Card title="Paste any HTML" icon="upload">
    Bringing a design from another tool (Figma export, Webflow, Lovable, Cursor, Gamma, etc.)? Paste the HTML straight into the chat — Claude strips scripts and container-breaking positioning, proposes which text should become tokens, and saves it with `create_page` or `update_page`.
  </Card>
</Columns>
...
The same coverage check applies when Claude saves a design you pasted in from another tool.
...
Most people build in a normal Claude chat. Claude Design is the **power-user path** when you want finer control over the visual design — but it has a gotcha worth knowing. For the full walkthrough see [Design in Claude Design](/claude-connector/claude-design).
...
Call get_variables(product_id) for the product's REAL token names, get_design_system() for the
saved brand colours/fonts, and get_page for existing pages to match. Never invent token names —
...
With that in place, ask Claude Design to *"use my Productised connector, read the real tokens and design system for product `<id>`, then design and save the output page."* It will pull your real tokens (`get_variables`), brand colours (`get_design_system`) and existing pages, design a single-file HTML page that matches, and save it live — no React detour, no manual port.

---

## Porting designs from Lovable, v0, or Cursor

You can bring in HTML from any design tool by pasting it into the chat. Claude cleans and validates the HTML before saving — strips scripts and external stylesheets, fixes container-breaking layout, detects placeholder text like "Maya" or "[Name]", and suggests the right tokens.

**How each tool's output translates:**

| Tool | Output type | Works in Productised? | How |
|------|-------------|----------------------|-----|
| Claude Design (connector in the chat) | Clean HTML saved via the connector | ✅ Yes | Say *"use this as my output page"* — Claude saves it. [Details →](/claude-connector/claude-design) |
| Claude Design — "Export → Standalone HTML (offline)" file | React app bundle | ❌ No — imports blank | Don't use it. Ask for *"a single static HTML file, no JavaScript"* instead |
| Cursor / any AI that writes HTML | Full HTML or fragment | ✅ Directly | Paste into Claude — it cleans it and saves via `update_page` |
...
| Figma / Webflow / Framer | Varies | ⚠️ Export HTML first | Export as HTML, paste it into the chat |
