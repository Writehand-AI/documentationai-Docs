...
description: How double-curly-brace tokens in landing pages, result pages, and prompts resolve per respondent — and which prefix to use where.
---

Productised uses `\{\{ token \}\}` syntax to inject dynamic values at render time. Six prefixes, each backed by a different source. Five of them resolve from the respondent's collected data (form answers, AI agent output, computed visualisations, workspace variables); the sixth — `url:` — resolves from the request URL itself, for outbound personalisation.
...
A respondent isn't logged in yet at the landing-page stage, so `form:`, `node:`, `object:`, and `chart:` don't have data to resolve from. Only the workspace-wide `custom:` tokens and the request-URL `url:` tokens work there.
