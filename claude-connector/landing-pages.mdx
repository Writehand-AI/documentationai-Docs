...
Every Productised product has a **welcome screen** — the first thing a respondent sees before the AI conversation starts. By default it's a clean title-page style with your product name. You can replace it with a fully custom HTML landing page designed in Claude.
...
<Columns cols="2">
  <Card title="set_welcome_html" icon="edit">
    Save a custom HTML landing page to a product. Sets the welcome style to "Claude Page" and renders an inline preview in the chat so you see exactly what respondents will see.
  </Card>
...
The landing page is essentially **static marketing HTML** with optional workspace variables — no respondent-specific data, because the respondent hasn't started yet.
...
Open the demo URL and you see the page as a respondent would — full-width on the chat surface, with the auto-appended "Get Started →" button at the bottom.
...
- **No respondent variables.** Only `{{ custom:* }}` workspace vars resolve here (because there's no respondent yet). Don't put `{{ form:* }}` or `{{ object:* }}` in a landing page — they won't fill.
