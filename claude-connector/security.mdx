...
- **Delete responses.** The connector cannot delete a lead. Deletion is not impossible, though — it happens in the app, deliberately: delete a single lead from the product's **Analytics → All Results** table, or set a retention period under **Settings → Client settings → Data retention** so leads are removed automatically after a set number of days. That split is the point: Claude can read and organise leads, but erasing someone's data is a decision you make in the dashboard.
