...
<Columns cols="2">
  ...
  <Card title="Marked as test runs" icon="eye-off">
    Runs from a demo URL are saved as test runs, marked **Test** in analytics. They show by default — switch on **Hide test runs** in Analytics to leave them out — and they never send lead emails, notifications, webhooks or Zapier data.
  </Card>
</Columns>

<Callout kind="warning">
**Test runs don't trigger integrations.** Runs from your product's demo link (ending in `?demo=true`) are test runs. They appear in your analytics, marked **Test**, but they never send lead emails, notifications, webhooks or Zapier data. To check your integration, use **Send Test** in **Product Settings → Webhook**, or complete the product from its live link.
</Callout>
...
**Test runs carry the demo flag through.** When you run through a product from its demo link, the conversation hands off to the result page with `?demo=true` still attached, so you land on your result even while the product is unpublished. Anyone opening the live URL of an unpublished product still sees nothing until you publish it.
