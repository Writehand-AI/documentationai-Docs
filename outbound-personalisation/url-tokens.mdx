...
Outbound Personalisation runs on a simple template syntax. Drop a `\{\{ url:fieldName | fallback \}\}` token anywhere in your product's user-facing text and the renderer substitutes the URL parameter's value at the moment a respondent loads the page.
...
- Referrer headers when the respondent clicks an outgoing link
