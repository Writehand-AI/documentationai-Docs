...
- Open the AI Product node and set **Collection Instructions → Language** to the language you want. On **Auto**, the product follows the language your instructions are written in, so English instructions give an English product.
...
- Building with Claude? Just say *"Set the language of [product name] to German."*

## Result page doesn't appear at the end of a test run

- The result page is part of the live product, so publish the product (Product Settings → Deployment, or ask Claude to *"publish [product name]"*) and run through it again from the live link.
- To test before publishing, use the product's demo link (the one ending in `?demo=true` that Claude shares). A run started from that link goes all the way through to the result page.
