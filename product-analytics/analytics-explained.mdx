...
description: >-
  Everything you need to understand how your AI product is performing — and how
  to improve it.
---
...
<Callout kind="info">
**Test runs** — runs from your product's demo link (`?demo=true`) — are included by default and marked **Test** in All Results. Switch on **Hide test runs** at the top of Analytics to leave them out of the charts, All Results, AI Insights, Talking Points and exports. The setting is saved for the whole workspace, so it also applies to the dashboard and the Brain map.
</Callout>
...
| **Outputs**         | Completed sessions that generated a result                                                    |
...
- **Top cost drivers** — which nodes in your workflow consumed the most credits this month (e.g. _AI Advisor 77%_, _Standard Output 21%_). If one node dominates and feels too expensive, tighten its prompt or move it to a smaller model.
...
If you have BYOK active (your own Anthropic key added in **Settings → AI & connections → Claude API Key**), the card switches to "**BYOK is on**" and the real-$ tiles show your actual Anthropic spend — not an equivalent. Plan credits are not consumed.

If you don't have BYOK, the footer suggests it: _"this product would cost you \~$0.84 this month direct to Anthropic"_ — a real number from your own usage, not an estimate. Add one in **Settings → AI & connections → Claude API Key**.
