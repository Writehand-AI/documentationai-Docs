...
title: Custom Domain
description: >-
  Serve every product in your workspace under your own domain with one DNS
  record and one cert.
---
...
- **An apex domain** like `acme.com` directly — uses A/AAAA records and means the apex won't be available for anything else (no website at `https://acme.com`). Only choose this if you're running Productised as your _whole_ business.
...
Back in settings, click **Check status**. This issues the HTTPS certificate if the domain doesn't have one yet, then shows the status. Once the certificate is active, the page shows `Status: Active ✓` and your products are live at their workspace-domain URLs.

<Callout kind="info">
Set the domain up by asking Claude? Claude saves it and gives you the DNS record, but the certificate is issued when you click **Check status** here in the app — so finish with this step either way.
</Callout>
...
New products get a slug from their name automatically. To change one, edit it next to the product on the **Custom Domain** page and click **Save**, or ask Claude:

> "Set the slug for my Leadership Readiness product to 'leadership-readiness'"

Slugs are unique within your workspace — if you pick one that's taken, `-2`, `-3` and so on is added automatically.
...
If you decide to stop using a workspace-wide domain, click **Remove** on the settings page (or ask Claude: _"remove my workspace domain"_). Products fall back to their default URLs — per-product custom domain if set, otherwise `https://app.productised.ai/c/<product-id>`. Nothing about the products themselves changes; just the hostname they're reachable under.
